...
//Interface
import { TGetTV } from "@/server/movies/output/movie.output";
interface Props {
    errorMessage?: string;
    value: string;
...
const Autocomplete = ({ errorMessage, value = "", onChange, movieId }: Props) => {
    ...
    //Trpc
    const { data, isFetching } = trpc.movie.getTv.useQuery(search);

    //Initializing Hook
    const { cursor, setCursor, ref } = useKeyboardNavigation<TGetTV>(data || [], (e) => {
        onChange(e.id);
        setOpen(false);
    }, open);

    useEffect(() => {
        const name = data?.find(a => a.id === value)?.name;
        setSearch(name || "");
                            ...
                            <Scroll className="max-h-[300px]" showAlways ref={ref}>
                                {data?.map((item, i) => (
                                    <div
                                        ...
                                        onClick={() => (onChange(item.id), setCursor(i), setOpen(false))}
                                    >
                                        {item.name}
                                    </div>
                                ))}
